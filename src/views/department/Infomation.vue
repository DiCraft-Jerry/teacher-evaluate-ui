      ...
      <el-table-column prop="cName" label="所属学院">
        <template slot-scope="scope">
          <el-tag
            size="small"
            type="danger"
            v-for="college in scope.row.collegeInfos"
            >{{ college.name }}</el-tag
          >
        </template>
      </el-table-column>
      ...
      <el-table-column prop="icon" width="260px" label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            v-if="hasAuth('department:infomation:evaluation')"
            >评教</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  created() {
    ...
    this.$axios.get("/department/infomation/batch").then((res) => {
  ...
  methods: {
    ...
    getInfomation() {
      ...
      this.$axios
        .get("/department/infomation/list", {
          params: {
    ...
    evaluationHandle(id) {
      ...
      this.teachId = id;
      this.$axios
        .get("/department/infomation/info/" + id + "/" + this.userInfoId)
        .then((res) => {
