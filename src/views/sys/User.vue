<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="icon" width="260px" label="操作">
        <template slot-scope="scope">
          ...
          <el-button type="text" @click="collegeHandle(scope.row.id)"
            >分配学院</el-button
          >
          <el-divider direction="vertical"></el-divider>

          <el-button type="text" @click="classHandle(scope.row.id)"
            >分配班级</el-button
          >
          <el-divider direction="vertical"></el-divider>
          <el-button
    ...
    <el-dialog
      title="分配学院"
      :visible.sync="collegeDialogFormVisible"
      width="600px"
    >
      <el-form :model="collegeForm">
        <el-tree
          :data="collegeTreeData"
          show-checkbox
          ref="collegeTree"
          :check-strictly="checkStrictly"
          node-key="id"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="collegeDialogFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="submitCollegeHandle('collegeForm')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitClassHandle('classForm')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      collegeDialogFormVisible: false,
      collegeForm: {},
      collegeTreeData: [],
  ...
  created() {
    ...
    this.$axios.get("/work/college/list").then((res) => {
      this.collegeTreeData = res.data.data.records;
    });

    this.$axios.get("/work/class/list").then((res) => {
      this.classTreeData = res.data.data.records;
    });
  },
  methods: {
    ...
    collegeHandle(id) {
      this.collegeDialogFormVisible = true;

      this.$axios.get("/sys/user/info/" + id).then((res) => {
        this.collegeForm = res.data.data;

        let collegeIds = [];

        res.data.data.collegeInfos.forEach((row) => {
          collegeIds.push(row.id);
        });

        this.$refs.collegeTree.setCheckedKeys(collegeIds);
      });
    },

    submitCollegeHandle(formName) {
      var collegeIds = this.$refs.collegeTree.getCheckedKeys();

      console.log(collegeIds);

      this.$axios
        .post("/sys/user/college/" + this.collegeForm.id, collegeIds)
        .then((res) => {
          this.$message({
            showClose: true,
            message: "恭喜你，操作成功",
          ...
          this.collegeDialogFormVisible = false;
        });
    },
    ...
    classHandle(id) {
      ...
      this.$axios.get("/sys/user/info/" + id).then((res) => {
        ...
        res.data.data.classInfos.forEach((row) => {
    ...
    submitClassHandle(formName) {
      var classIds = this.$refs.classTree.getCheckedKeys();

      console.log(classIds);

      this.$axios
        .post("/sys/user/class/" + this.classForm.id, classIds)
        .then((res) => {
          this.$message({
            showClose: true,
            message: "恭喜你，操作成功",
