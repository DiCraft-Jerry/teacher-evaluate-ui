      ...
      <el-table-column prop="icon" width="260px" label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            v-if="hasAuth('teacher:infomation:evaluation')"
            >评教</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  created() {
    ...
    this.$axios.get("/teacher/infomation/batch").then((res) => {
  ...
  methods: {
    ...
    evaluationHandle(id) {
      ...
      this.teachId = id;
      this.$axios
        .get("/teacher/infomation/info/" + id + "/" + this.userInfoId)
        .then((res) => {
    ...
    submitPermFormHandle() {
      ...
      this.$axios
        .post(
          "/teacher/infomation/options/" + this.userInfoId + "/" + this.teachId,
          test
        )
        .then((res) => {
          this.$message({
