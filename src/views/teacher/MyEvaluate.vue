<template>
  <div>
      ...
      <el-table-column prop="icon" width="260px" label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            v-if="hasAuth('teacher:myEvaluate:evaluation')"
            >评教</el-button
          >
        </template>
      </el-table-column>
    ...
    <!-- 开始评教 -->
    <el-dialog title="自我评教" :visible.sync="permDialogVisible" width="600px">
      <el-container>
        <el-main>
          <el-row>
            <el-col :span="24">
...
export default {
  ...
  created() {
    ...
    this.$axios.get("/teacher/myEvaluate/batch").then((res) => {
  ...
  methods: {
    ...
    evaluationHandle(id) {
      ...
      this.teachId = id;
      this.$axios
        .get("/teacher/myEvaluate/info/" + id + "/" + this.userInfoId)
        .then((res) => {
    ...
    submitPermFormHandle() {
      ...
      this.$axios
        .post(
          "/teacher/myEvaluate/options/" + this.userInfoId + "/" + this.teachId,
          test
        )
        .then((res) => {
          this.$message({
